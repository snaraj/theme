...
pkgs.testers.runNixOSTest {
  name = "theme-cli";
  nodes.machine = {
    environment.systemPackages = [ theme pkgs.curl pkgs.python3 pkgs.bashInteractive pkgs.zsh pkgs.fish pkgs.nushell ];
  ...
  testScript = ''
    ...
    machine.succeed("su - tester -c 'cp ${./cli_maintenance_test.py} checks/tests/cli_maintenance_test.py && CI=true python3 -I -B checks/tests/cli_maintenance_test.py ${theme}/bin/theme --require-all-shells'")
  '';
}
